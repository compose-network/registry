...
import (
	...
	comp "github.com/compose-network/registry/networks/hoodi"
...
func main() {
	...
	// Load compose chains from embedded TOMLs
	chains, err := comp.ComposeChains()
	if err != nil {
	...
	for _, c := range chains {
		slug := c.Name // compose Name matches slug (e.g. rollup-a)
		expectID := int64(c.ChainID)
		...
		if c.Genesis.L2Time != 0 {
			ts, err := anyToInt64(g.Timestamp)
			if err != nil {
				fatalf("%s timestamp parse: %v", genPath, err)
			}
			if ts != int64(c.Genesis.L2Time) {
				fatalf("%s timestamp=%d, want %d", genPath, ts, c.Genesis.L2Time)
			}
		}
	}
